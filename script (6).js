...
let jobs = [
    { id: 1, title: "Senior Software Engineer", company: "TechCorp", salary: "PKR 250,000", location: "Karachi" },
    { id: 2, title: "Marketing Manager", company: "Global Brands", salary: "PKR 150,000", location: "Lahore" },
    { id: 3, title: "Data Analyst", company: "FinTech Solutions", salary: "PKR 120,000", location: "Islamabad" }
];
...
function renderFeaturedJobs() {
    ...
    container.innerHTML = jobs.map(job => `
        <div onclick="viewJob(${job.id})" class="job-card bg-white rounded-3xl p-6 shadow cursor-pointer">
            ...
            <p class="text-orange-600 font-bold mt-4">${job.salary}</p>
...
function renderCategories() {
    const cats = ["Software Development", "Marketing", "Finance", "Sales", "HR"];
    const container = document.getElementById('categories');
    container.innerHTML = cats.map(cat => `
        <div onclick="filterCategory('${cat}')" class="bg-white p-6 rounded-3xl text-center cursor-pointer hover:bg-orange-50 transition">
...
function searchJobs() {
    ...
    container.innerHTML = filtered.map(job => `
        <div onclick="viewJob(${job.id})" class="job-card bg-white rounded-3xl p-6 shadow cursor-pointer">
            ...
            <p class="text-orange-600 font-bold">${job.salary}</p>
        </div>
    `).join('');
}
